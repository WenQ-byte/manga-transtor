...
function regionBox(region) {
  const xs = region.box.map((point) => point[0])
  const ys = region.box.map((point) => point[1])
...
function regionLayoutBox(region, active = false) {
  return region.render_box ? regionBox({ box: region.render_box }) : regionBox(region)
}
...
function PlannedRegionText({ region, text, color, fontFamily, fontWeight, active = false, live = false }) {
  const plan = region.render_layout
  const box = live ? regionBox(region) : (region.render_box ? regionBox({ box: region.render_box }) : regionBox(region))
  ...
  const groupStyle = { transformBox: 'view-box', transition: live ? 'none' : 'transform 150ms ease-out' }
  if (plan.direction === 'v') {
    return <g transform={`translate(${box.x} ${box.y})`} style={groupStyle}>
  ...
  return <g transform={`translate(${box.x} ${box.y})`} style={groupStyle}>
    {(plan.lines || [text]).map((line, index) => <text key={index} x={Number(plan.center_x || box.width / 2)} y={Number(plan.top || 0) + index * (Number(plan.line_height || fontSize * 1.4) + Number(plan.spacing || 0))} textAnchor="middle" dominantBaseline="hanging" style={common}>{line}</text>)}
  </g>
}

function LiveTextLayer({ regions, selected, draft, fontSize, fontFamily, fontWeight, defaultTextColor, textColor, fontOptions, imageSize, draggingIndex = null, className = '' }) {
  return <svg className={`pointer-events-none absolute inset-0 h-full w-full ${className || 'z-[5]'}`} viewBox={`0 0 ${imageSize.width} ${imageSize.height}`} preserveAspectRatio="none" aria-label="实时文字图层">
    {regions.map((region) => {
      const active = selected?.index === region.index
      const pending = draggingIndex === region.index
      const text = active ? draft : (region.translated || '')
      if (!text.trim()) return null
      const box = pending ? regionBox(region) : regionLayoutBox(region, active)
      ...
      if (region.render_layout && (!active || text === region.translated)) {
        return <PlannedRegionText key={region.id || region.index} region={region} text={text} color={color} fontFamily={option?.css_family || 'Microsoft YaHei'} fontWeight={active ? fontWeight : (region.font_weight || 400)} active={active} live={pending} />
      }
...
function ImageLightbox({ value, onClose, onUpdated, onDownload, downloading }) {
  ...
  async function persistRegionBox(index, box, before) {
    const current = regions.find((item) => item.index === index)
    if (!current) return
    ...
    setSaving(true)
    try {
      const response = await editTaskRegion(value.taskId, index, isSelected ? (draft || current.translated) : current.translated, { fontSize: current.font_size != null ? (isSelected ? fontSize : current.font_size) : null, fontFamily: isSelected ? fontFamily : (current.font_family || ''), fontWeight: isSelected ? fontWeight : (current.font_weight ?? 400), color: isSelected ? (defaultTextColor ? '' : textColor) : (current.color ? `#${current.color.map((part) => Number(part).toString(16).padStart(2, '0')).join('')}` : ''), ...box })
      ...
      setSelected((item) => item?.index === index ? { ...item, box: response.box || item.box, render_box: response.render_box || item.render_box, render_layout: response.render_layout || item.render_layout, font_size: response.font_size, default_font_size: response.default_font_size || item.default_font_size } : item)
      setCacheBust(Date.now()); setError(response.warning || ''); onUpdated(value.taskId)
    } catch (err) { updateRegionBox(index, before); setError(`文本框更新失败，已恢复原位置：${err.message}`) }
    finally { setSaving(false); setDraggingIndex((current) => (current === index ? null : current)) }
  }
  ...
  function moveCanvasPan(event) {
    ...
    if (!p) return
    event.preventDefault()
    if (Math.hypot(event.clientX - p.x, event.clientY - p.y) > 4) panMovedRef.current = true
    setPan({ x: p.panX + (event.clientX - p.x), y: p.panY + (event.clientY - p.y) })
  }
  ...
  function startRegionInteraction(event, region, handle = '') {
    event.preventDefault(); event.stopPropagation(); selectRegion(region); setDraggingIndex(region.index)
    ...
    regionInteractionRef.current = { index: region.index, handle, start, startPointer: pointer(event.clientX, event.clientY), before: start, pointer }
    event.currentTarget.setPointerCapture?.(event.pointerId)
  }
  ...
  useEffect(() => {
    const move = (event) => {
      ...
      const next = { ...start }
      if (!handle) {
        next.x = Math.max(-start.width + REGION_MIN_VISIBLE, Math.min(imageSize.width - REGION_MIN_VISIBLE, start.x + dx))
        next.y = Math.max(-start.height + REGION_MIN_VISIBLE, Math.min(imageSize.height - REGION_MIN_VISIBLE, start.y + dy))
      } else {
        if (handle.includes('e')) next.width = Math.max(REGION_MIN_SIZE, Math.min(imageSize.width * REGION_MAX_SIZE_RATIO - start.x, start.width + dx))
        if (handle.includes('s')) next.height = Math.max(REGION_MIN_SIZE, Math.min(imageSize.height * REGION_MAX_SIZE_RATIO - start.y, start.height + dy))
    ...
    const up = () => { const interaction = regionInteractionRef.current; if (!interaction) return; regionInteractionRef.current = null; if (interaction.last && JSON.stringify(interaction.last) !== JSON.stringify(interaction.before)) { persistRegionBox(interaction.index, interaction.last, interaction.before) } else { setDraggingIndex(null) } }
    window.addEventListener('pointermove', move, { passive: false }); window.addEventListener('pointerup', up)
    return () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up) }
  }, [imageSize, regions, value, draft])
        ...
        <motion.div ref={lightboxRef} className="fixed inset-0 z-[80] flex items-center justify-center bg-black/85 p-3 backdrop-blur-sm"
          ...
          <div className="flex max-h-[98vh] max-w-[99vw] flex-col overflow-hidden rounded-xl border border-white/10 bg-black/75 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div ref={viewportRef} className={`relative min-h-0 overflow-hidden flex items-center justify-center ${editMode && !eraseMode ? (spacePressed ? 'cursor-grabbing' : 'cursor-grab') : ''}`} onWheel={zoomCanvas} onPointerDown={startCanvasPan} onPointerMove={moveCanvasPan} onPointerUp={endCanvasPan} onPointerCancel={endCanvasPan}>
              ...
              <div ref={imageCanvasRef} className="relative" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${canvasZoom})`, transformOrigin: 'center' }} onClick={(event) => { if (!eraseMode && !panMovedRef.current && event.target === event.currentTarget) setSelected(null) }}>
                <motion.img src={fontPreviewUrl || `${value.url}${cacheBust ? `?v=${cacheBust}` : ''}`} alt={`${value.filename} 翻译结果大图`} className="block max-h-[calc(98vh-48px)] max-w-[99vw] select-none object-contain"
                ...
                {editMode && imageSize.width > 1 ? <LiveTextLayer regions={regions} selected={selected} draft={draft} fontSize={fontSize} fontFamily={fontFamily} fontWeight={fontWeight} defaultTextColor={defaultTextColor} textColor={textColor} fontOptions={fontOptions} imageSize={imageSize} draggingIndex={draggingIndex} className={eraseMode ? 'z-40' : 'z-[5]'} /> : null}
                ...
                {editMode ? <svg className={`absolute inset-0 z-10 h-full w-full touch-none ${eraseMode ? 'pointer-events-none' : ''}`} viewBox={`0 0 ${imageSize.width} ${imageSize.height}`} preserveAspectRatio="none" aria-label="文本框编辑画布" onClick={(event) => { if (!eraseMode && !panMovedRef.current && event.target === event.currentTarget) setSelected(null) }}>
...
function EmptyConversation() {
  return (
    <motion.div initial={{ opacity: 0, y: 14, scale: 0.96, filter: 'blur(10px)' }}
      ...
      <ComicXWordmark shiny className="h-36 w-auto sm:h-48" />
    </motion.div>
  )
}
...
function TaskCard({ item, displayIndex, downloading, retrying, onDownload, onRetry, onView }) {
  ...
  useEffect(() => { setPreviewFailed(false) }, [resultUrl])
  return (
          ...
          {previewFailed ? (
            ...
            <div className="group relative block w-full overflow-hidden rounded-xl border border-line bg-[#202020]">
              <img src={resultUrl} alt={`${filename} 翻译结果`} loading="lazy" decoding="async"
                onError={() => setPreviewFailed(true)} onClick={onView} className="max-h-[720px] w-full cursor-zoom-in object-contain" />
