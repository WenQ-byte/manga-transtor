...
class TranslationTaskManager:
    ...
    def restore_text_region(self, task_id: str, region_index: int) -> dict:
        ...
        base_image = Image.open(io.BytesIO(state["cleaned_image"])).convert("RGB")
        original_image = Image.open(original_path).convert("RGB")
        if original_image.size != base_image.size:
            raise ValueError("原始图片尺寸与编辑图片不一致，无法恢复")
        restore_mask = Image.new("L", base_image.size, 0)
        drawer = ImageDraw.Draw(restore_mask)
        ...
        base_image.paste(original_image, mask=restore_mask)
        restored_bytes = io.BytesIO()
        base_image.save(restored_bytes, format="PNG")
        restored_cleaned = restored_bytes.getvalue()
        with self._pipeline_lock:
            pipeline = self._get_pipeline()
            render_path = self.files.save_state(restored_cleaned)[1]
            try:
                image_bytes = pipeline.renderer.render(Path(render_path), remaining, target_lang=task.get("target_lang") or "zh")
            finally:
                Path(render_path).unlink(missing_ok=True)
        new_state_io = io.BytesIO()
        state["regions"] = remaining
        state["cleaned_image"] = restored_cleaned
        pickle.dump(state, new_state_io)
        new_state_rel, _ = self.files.save_state(new_state_io.getvalue())
        _, new_result = self.files.save_result(image_bytes, ".png")
        meta = dict(task.get("meta") or {})
    ...
    def get_edit_regions(self, task_id: str) -> dict:
        ...
        for items in grouped.values():
            ...
            uses_user_bounds = any(
                getattr(region, "source", "ocr") == "manual"
                or getattr(region, "layout_bounds_override", False)
                for region in members
            )
            bounds = next((region.group_bounds for region in members if region.group_bounds), None) if uses_user_bounds else None
            if not bounds:
    ...
    def edit_task_region(self, task_id: str, region_index: int, translated: str, font_size: int | None = None, font_family: str = "", color: str = "", font_weight: int | None = None, x: int | None = None, y: int | None = None, width: int | None = None, height: int | None = None) -> dict:
        task = self.db.task_get(task_id)
        if not task or task.get("status") != "completed":
