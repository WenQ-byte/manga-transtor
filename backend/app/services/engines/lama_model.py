"""LaMa（FFC 生成网络）模型定义，vendor 自 manga-image-translator
（manga_translator/inpainting/inpainting_lama_mpe.py，保留推理所需最小集）。
权重：lama_large_512px.ckpt（big-lama 架构：sigmoid 输出、FFC + 18 个 resnet block）。
仅依赖 torch；判别器/MPE（训练用）已省略。
"""
...
class LamaFourier:
    """LaMa 推理封装：big-lama（sigmoid 输出，输入 0-1，输出按 mask 回填原图）"""
...
def load_lama_large(model_path: str, device: str = "cpu") -> LamaFourier:
    """加载 lama_large_512px.ckpt（big-lama 生成器）"""
