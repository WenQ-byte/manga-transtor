...
async def edit_translation_region(
    ...
    move_only: bool = Form(False),
    manager: TranslationTaskManager = Depends(get_manager),
):
    try:
        return manager.edit_task_region(task_id, region_index, translated, font_size, font_family, color, font_weight, x, y, width, height, move_only=move_only)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
