...
description: |
  学习报告上传技能 — 自动扫描实训记录中的学生情况和日志 Markdown 文件，
...
- `.` — 工作区根目录作为参数传入（脚本会在此目录下扫描 `实训记录/学生情况/` 和 `实训记录/日志/`）

脚本会自动完成：
- 扫描 `实训记录/学生情况/` 和 `实训记录/日志/` 下的 `.md` 文件
...
```json
{
  "uploaded_files": [
    {
      "filename": "软工1班-25010001-张三.md",
      "relative_path": "实训记录/学生情况/软工1班-25010001-张三.md",
