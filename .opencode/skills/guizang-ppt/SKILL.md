...
description: 生成横向翻页网页 PPT（单 HTML 文件）或可编辑的 .pptx，含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"，或需要"生成可编辑的 ppt"时使用。
icon: presentation
tags:
  - ppt
  - presentation
...
### Step 7 · 生成可编辑的 .pptx（可选）

如果用户需要可编辑的 PowerPoint 文件（用于 PowerPoint/Keynote/WPS 进一步编辑），可以使用内置脚本将 HTML 转换为 .pptx：
...
# 风格 A + 墨水经典主题（默认）
python scripts/html_to_pptx.py 项目/XXX/ppt/index.html 项目/XXX/ppt/output.pptx

# 风格 A + 指定主题
python scripts/html_to_pptx.py 项目/XXX/ppt/index.html 项目/XXX/ppt/output.pptx A 靛蓝瓷

# 风格 B + IKB蓝主题
python scripts/html_to_pptx.py 项目/XXX/ppt/index.html 项目/XXX/ppt/output.pptx B IKB蓝
```

**方案二：浏览器方案（高精度，推荐）**

使用 Playwright + pptxgenjs，通过浏览器 DOM 的 `getBoundingClientRect()` 获取精确位置，还原度更高。
...
# 生成 PPTX
node html_to_pptx_browser.js 项目/XXX/ppt/index.html 项目/XXX/ppt/output.pptx ikb
```
...
| 方案 | 精度 | 依赖 | 适合场景 |
|------|------|------|----------|
| Python 脚本 | ★★★☆☆ CSS 类名推断 | python-pptx | 批量处理、无需浏览器 |
| 浏览器方案 | ★★★★★ getBoundingClientRect | Node.js + Playwright | 高还原度需求 |

**详细说明**见 `references/pptx-guide.md`。
...
│   └── html_to_pptx.py         ← HTML → .pptx 转换脚本（生成可编辑 PowerPoint）
└── references/
    ...
    ├── pptx-guide.md         ← HTML → .pptx 转换指南（生成可编辑 PowerPoint）
